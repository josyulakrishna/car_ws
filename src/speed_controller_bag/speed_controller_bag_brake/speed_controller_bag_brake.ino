...
void accelerate()
{
  ...
  double grad_vel = tar_vel - prev_tar_vel;
  prev_tar_vel = tar_vel;
  vel_err = tar_vel - cur_vel;   ////+0.2778
  rem_t = rem_t + 0.110;
  if (rem_t >= tar_t)
    rem_t = 0;
  ...
  {
    nh1.loginfo("speed");
    req_torq = R * mass * (vel_err / (tar_t) + g * sin(gradient_angle) + g * mu_k * cos(gradient_angle));
