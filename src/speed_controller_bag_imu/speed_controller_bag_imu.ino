...
double tar_t=0, tar_omega=0, gradient_angle=0, rem_t=0, req_torq=0, temp_brake=0 ;
...
void get_imu(const sensor_msgs::Imu& imu_msg)
{
    nh1.loginfo("imusub");
//    tf::Quaternion q(imu_msg->pose.pose.orientation.x,imu_msg->pose.pose.orientation.y,imu_msg->pose.pose.orientation.z,imu_msg->pose.pose.orientation.w);
//    tf::Matrix3x3 m(q);
//    
//    m.getRPY(roll, pitch, yaw);
  ...
  double sinp = +2.0 * (imu_msg.orientation.w * imu_msg.orientation.y - imu_msg.orientation.z * imu_msg.orientation.x);
  if (fabs(sinp) >= 1)
    pitch = copysign(M_PI / 2, sinp); // use 90 degrees if out of range
  else
    pitch = asin(sinp);

  gradient_angle = pitch;

  if(gradient_angle>0.01)
   nh1.loginfo("angle"); 
...
ros::Subscriber<sensor_msgs::Imu> sub1("imu/data", &get_imu);
...
//    dac.setVoltage(dac_count, false);
    ...
    vel_err = tar_vel+0.2778 - cur_vel;
    rem_t = rem_t+0.05;
    if(rem_t >= tar_t)
      rem_t=0;    
    ...
    { 
      nh1.loginfo("speed");       
      req_torq = R*mass*(vel_err/(tar_t-rem_t) + g*sin(gradient_angle) + g*mu_k*cos(gradient_angle));
