...
#include <trajectory_plot/output.h>
...
trajectory_plot::output odom_pack; 
ros::Publisher pub_val("odom_stat", &odom_pack);
...
void position_calc()
{
  odom_pack.ref_velocity= cur_vel*3.6;
   
  delf = encoder0Pos * ((35*pi/180)/4096);  
  beta = atan((1.2*tan(delf))/1.9);
  theta0=((cur_vel)*cos(beta)*tan(delf)/1.9)*time_step;    
  if((theta0*1000)> 0.1)  
    theta = (thetao + theta0); 
  x = x + cur_vel*cos(theta)*time_step;
  y = y + cur_vel*sin(theta)*time_step;
  odom_pack.x_pos=x;
  odom_pack.y_pos=y;
  odom_pack.heading=theta*180/pi;
  odom_pack.header.stamp=nh1.now();
  odom_pack.header.frame_id="/odom";
  thetao=theta;
}
